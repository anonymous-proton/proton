...
RUN set -eux \
    && if command -v micromamba >/dev/null 2>&1; then \
        ...
        micromamba run -n "${env_name}" python -m pip install --no-cache-dir pynvml tqdm hydra-core psutil pyyaml grpcio grpcio-tools protobuf ; \
    elif command -v python >/dev/null 2>&1; then \
        python -m pip install --no-cache-dir pynvml tqdm hydra-core psutil pyyaml grpcio grpcio-tools protobuf ; \
    elif command -v python3 >/dev/null 2>&1; then \
        python3 -m pip install --no-cache-dir pynvml tqdm hydra-core psutil pyyaml grpcio grpcio-tools protobuf ; \
    else \
