...
class ProfileConfig:
    ...
    create_weight_aggregated_report: Optional[bool] = None
...
def run_ncu_aggregator(cfg: ProfileConfig, paths: ProfilePaths) -> None:
    from profile.utils import ncu_time_weighted_aggregator

    print(f" Generating aggregated report from {paths.kernel_csv} to {paths.ncu_dir / 'aggregated_report.csv'}")
    ncu_time_weighted_aggregator.process_ncu_timeline(timeline_path= paths.kernel_csv, rep_dir= paths.ncu_dir, output_path= paths.ncu_dir / "aggregated_report.csv")
...
def run_profile(cfg: ProfileConfig) -> None:
    ...
    print(f"TOP-K THRESHOLD SET TO: {cfg.threshold}")
    print(f"DO_CREATE_WEIGHT_AGGREGATED_REPORT: {cfg.create_weight_aggregated_report}")
    ...
    try:
        if paths.kernel_csv.exists():
            ...
            kernels = extract_top_kernels(cfg, paths, threshold=cfg.threshold)
    ...
    if cfg.create_weight_aggregated_report:
        run_ncu_aggregator(cfg, paths)
...
def _config_from_globals() -> ProfileConfig:
    ...
    threshold = globals().get("threshold")
    create_weight_aggregated_report = globals.get("create_weight_aggregated_report")
    ...
    return ProfileConfig(
        ...
        threshold=threshold,
        create_weight_aggregated_report=create_weight_aggregated_report,
    )
