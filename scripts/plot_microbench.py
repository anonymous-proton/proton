...
PROFILE_DATA_ROOT (default: /mnt/nfs/new/proton/profile/{raw,results,parsed_csv})
  <component>/
...
Usage:
  python scripts/plot_microbench.py \
    --data-root /mnt/nfs/new/proton/profile/parsed_csv \
    --experiments profiling/configs/experiments/minimal.yaml \
    --outdir profiling/plots/outputs

Boundary note:
  This utility is for offline diagnostics/reporting. Level and Nsight (`nsys`/`ncu`)
  merge heuristics here are not part of the canonical runtime submit contract.
"""
...
try:
    ...
    DATA_ROOT = Path("/mnt/nfs/new/proton/profile/results")
    RAW_DATA_ROOT = Path("/mnt/nfs/new/proton/profile/raw")
...
def collapse_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    using offline metric-specific level preferences:
